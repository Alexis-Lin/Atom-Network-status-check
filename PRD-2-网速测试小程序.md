...
- **A4.7 波形图规范（时间密度 · 实时刷新 · 配色）**：
  ...
  - **屏缘环两种用法，靠宽度区分**：检测中 = 旋转进度环（宽约 10px，蓝）；结果页 = **静止 2px 灯色细环**（与结论标题同色；仅绿 / 黄 / 红结果页，首页与失败态无环）。**决策记录**：V1.0 曾整体取消状态色环——10px 粗环与「bars + 标题 + 指标着色」冗余、视觉份量过重、挤压顶排元素；V1.2 以 2px 细环回归（用户定稿）：不占版面、不碰排版，粗细 5 倍差让「粗环旋转 = 测速中 / 细环静止 = 结果灯色」两条语义互不混淆；
