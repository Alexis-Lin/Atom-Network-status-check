...
- **A4.1 首页 = 纯入口（测速前）**：灯色 bars + 一句话结论 + **当前 Wi-Fi 名（SSID）** + 副行数据时效（「刚刚更新」≤60 秒 /「N 分钟前测」≤5 分钟）+ 主按钮「测速」+ 按钮下方**无边框文字入口「这是什么？」**（三级文字按钮，样式与检测中页「取消」同级：24px 灰、无边框——不与主按钮争视觉；点击进 FAQ 页，见 A4.9；从未测速空态同样保留此入口——新用户最需要它）。SSID 必须外显——用户点进来看「网好不好」时往往不知道自己连的是哪个 WiFi，灯色结论必须挂在具体网络名下才可归因（与 B2 按 SSID 缓存一致）。**不展示任何数字指标**——常驻探测没有真实带宽，数字只在测速后出现，「测速前 = 入口、测速后 = 报告」天然可区分；缓存超 5 分钟不显示旧灯色，直接「检测中…」并刷新；
...
- **A4.9 FAQ 说明页（首页可见入口 + 长按快捷方式）**：回答「为什么产品要自带网络测试」「和自己用测速软件有什么区别」等困惑（文案单一来源：中文 [`FAQ-网速测试常见问题.md`](./FAQ-网速测试常见问题.md) / 英文 [`FAQ-SpeedTest-EN.md`](./FAQ-SpeedTest-EN.md)，含屏上精简版字数预算；**隐私口径保守**——不描述数据流向与内容，上/下行仅解释为发送 / 接收速度）。交互规则：
  - **主入口（可见）**：首页测速按钮下方无边框文字按钮**「这是什么？」**（见 A4.1）——**决策记录**：原方案仅长按隐藏入口 + 首次一次性提示，评审判定发现成本仍过高（长按不是消费级设备的常识手势）；改为可见文字入口后，一次性提示随之取消（入口可见即无需教育），提示位让给更干净的首页；
  - **快捷方式（保留）**：任意页面长按非按钮 / 非报告卡交互区的背景约 0.8s（+ 轻震动反馈）同样调起——照顾结果页 / 失败态下就近查看的场景；
  - **形态**：全屏页（用户主动调起，不算打扰）——标题「关于网速测试」28px + 可滚动 Q&A 列表（问题 21 白加粗 / 答案 21 灰，答案 ≤3 行；**FAQ 为长文阅读页，正文用例外小一档 21px**，全局三档制不变，例外仅限本页）；**右滑退出**或点底部实底按钮「OK 好的」返回，**返回原页面且状态保留**（测速中长按不中断任务，FAQ 关闭后回到进行中的测速页）；
  - 失败态与客服话术中可引导「点首页『这是什么？』或长按屏幕查看说明」；
  - **页底结构**：FAQ 列表末尾依次为——跳转项「怎么连手机热点」→ 系统帮助页（路由由客户端配置，页内含 iPhone「最大兼容」开关说明；配合 Q6「信号差建议连手机 5G 热点」的引导闭环）→ 实底按钮「OK 好的」（点击返回）→「右滑可随时退出」提示（15px）；
  ![FAQ 页圆屏交互原型：可滚动 Q&A + 页底跳转项 + 上下渐隐](./images/界面-FAQ圆屏原型.png)

  - **边界**：长按判定与滚动 / 点按手势互斥（位移超阈值即取消）；**右滑退出手势与纵向滚动互斥**（横向位移主导才判退出）；按钮、波形卡、历史列表等可交互区域内长按不触发（避免误出）；FAQ 页内容静态本地内置，无网络依赖（断网也能看——断网场景恰恰是最需要它的时刻；跳转项目标为系统页，同样离线可达）。
