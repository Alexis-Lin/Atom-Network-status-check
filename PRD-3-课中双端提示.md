...
| 范围 | ATOM 设备端课中提示（抽屉 / 重试卡 / 中断页）· 存量弹窗收敛 · 带宽保护 · 手机 App 课中 · 启动加载页 |
...
![设备端课中三组件：Toast 三态 / 重试 UI 两态 / 阻断 UI 两归因变体](./images/界面-设备端课中.png)
...
**组件模型（定稿）**：课中提示只有三个 UI 组件，**组件 = 打扰级别，信息 = 内容参数**——同一组件复用于不同消息，用户看到组件形态就知道该不该动手：

| 组件 | 打扰级别 | 形态特征 | 承载内容（实例） | 出口 |
| --- | --- | --- | --- | --- |
| **抽屉**（轻度） | 非阻断 | **从底缘升起 → 停留约 3s → 收回**、零按钮、冷却 300s + 每课 ≤2 次 | 「网络一般 · 已降低画质」/「你的 / 课程画面可能卡顿」（红 bars 即「网络较差」，文案只说方向）/「网络已恢复」 | 自动收回，无状态 |
| **重试 UI**（重试卡） | 半阻断 | 屏幕中央双行卡、**系统自救中**、进度可见（第 N 次）、无按钮、无需操作 | 「网络中断 · 正在自动重连…」（画面定格）/「课程数据加载失败 · 正在重试…」（画面继续） | **只有两个**：成功 → 静默收起（+恢复抽屉）；超时/耗尽 → 升级阻断 UI |
...
| --- | --- | --- | --- | --- |
| 抽屉 | **穿透式底部抽屉**：比屏幕大、从屏下拉上来的面板——顶部两角 r40 完整可见，底边与两侧下段延伸出圆屏外（被圆缘裁掉），贴底 | mini bars（灯色）+ 单行文案，内容行居可见区中部 | 宽 360px、可见高 ≈ 142px；文案 24px、≤10 全角 | 升起 0.45s（ease-out）→ 停留约 3s → 收回 0.45s（ease-in）；冷却 300s、≤2 次/课（恢复不计入） |
| 重试 UI | **屏幕中央双行胶囊卡**——挡在内容前但不占满屏，正是「半阻断」的空间表达 | 第 1 行事件（「网络中断」/「课程数据加载失败」）+ 第 2 行蓝色进度（「⟳ 自动重连中（第 N 次）…」/「⟳ 重试中（1/2）…」，**有限次数外显**，耗尽即升级） | 两行均 24px、事件行 ≤8 全角；1.5px 白 16% 描边——与报告卡同一视觉语言：**描边 = 进行中的容器** | 持续显示至成功（静默收起 + 恢复抽屉）或超时/耗尽升级；spinner 匀速旋转；断线变体画面定格调暗 50%，数据变体画面继续 |
| 阻断 UI | 全屏 | 归因图标（断网 = 扇形断开 / 弱网 = 红 1 格 bars）→ 标题 32「课程已暂停」→ 归因副行 24 灰 → 重试进度行 24 蓝 → 实底按钮「WiFi 设置」 | 沿用全局三档字号（32/24/15）与按钮规范 | 恢复 → 自动回课 + 恢复抽屉 |

![抽屉出现与退出动效：从屏下拉上来 → 停留 3s → 退回去](./images/动效-课中抽屉.gif)

**位置语义（打扰级别的空间表达）**：**底缘抽屉 = 轻度瞬时** → **居中卡 = 中度持续**（挡在内容前）→ **全屏 = 阻断**。侵入面积随级别递增，位置本身就是语义。**课中无独立常驻角标（评审决议）**：网络信号 icon（bars）**永远跟随内容容器出现**——抽屉内、重试卡内、阻断页内；单独挂在画面顶部的状态图标属于常驻噪音，且与「提示即事件」冲突，灯的常驻表达由课前入口（PRD-1/2）承担。

**字号标准（探索结论）**：沿用全局三档（32/24/15），课中只动用两档——**32 仅用于全屏页标题**（阻断 UI「课程已暂停」），**24 用于一切胶囊文案、进度行、按钮**；15 元信息层课中无内容。**一致性原则：字号跟随内容层级，不跟随打扰级别**——同一句「网络较弱」在轻度 toast 和中度重试卡里字号相同（24），升级感全部由容器形态与位置表达（贴底单行 → 居中双行 → 全屏）。若字号随级别放大，同一句文案会出现三种大小，且「大字 = 重要」会与「全屏标题 32」的既有语义打架；中度级别的信息增量靠**加第二行**（进度），不靠加大字号。

**互斥规则**：重试卡显示期间不再弹抽屉（状态已在屏）；阻断 UI 显示期间其余组件全部不出——任意时刻屏上最多一个提示组件。

配套两条硬规则：① **单向升级**——事件只能沿 抽屉 → 重试卡 → 阻断页 变重，绝不反向（用户看到重试卡就知道"系统在忙"，看到阻断就知道"该我了"）；② **恢复即回落**——任何一级在网络恢复时自动消退，不需要用户确认。课中没有第 0 层常驻角标（评审决议，见上）——无事件时屏上没有任何网络元素。
...
```
                       ┌────────────────────────────────────────────────┐
 onNetworkQuality ──►  │ 抽屉（轻度）  灯色下降沿弹一次：升起→3s→收回      │  轻提醒
 (worst(local,remote)) │              bars 随抽屉出现，无事件时屏上无网络元素│
                       └────────────────────────────────────────────────┘
 onDisconnected ────►  重试卡「网络中断 · 自动重连中…」（半阻断，画面定格）
      └─ 重连超 30s ►  ┌────────────────────────────────────────────────┐
...
![优化后课中提示形态：底部抽屉 / 两种重试卡 / 中断页双归因变体 + 现状映射表](./images/优化后-课中提示形态.png)
...
| `onDisconnected` | 立即 | 居中重试卡「网络中断 · 自动重连中…」，画面定格 | 重试卡 |
| 断线持续 | 重试卡超 **30s** | 升级 Tier 2 全屏中断页（**断网归因**，自动重试可见，唯一按钮 WiFi 设置） | 阻断 |
| `onConnected` | 任意 | 收重试卡 / 中断页 + 抽屉「网络已恢复」 | 抽屉 |
| HTTP 可延后请求超时 | 打点 / 上报 / 组间上传 | 静默重试 ≤2 次 → 本地缓存恢复后补传；**永不上屏** | 0 |
| HTTP 关键请求超时 | 进课鉴权 / 课程资源 | 第 1 次静默重试 → 第 2 次带重试卡「课程数据加载失败 · 重试中…」→ 耗尽 Tier 2（**弱网归因**） | 0→重试卡→阻断 |
...
| 级别 | 例子 | 失败处理 |
| --- | --- | --- |
| **关键**（阻断课程流程） | 进课鉴权、课程资源拉取 | 静默自动重试 1 次（复用 `pending_ctx` 重放）→ 仍失败：断网 → Tier 2；超时 → 重试卡「课程数据加载失败 · 重试中…」再试 1 次 → 仍失败 → Tier 2 |
...
| `notify_trtc_disconnected`（1 次即跳断网页） | **改造** | 先重试卡「自动重连中…」+ 计时；**超 30s** 才 Tier 2（SDK 有自动重连，别抢跑） |
...
新增状态字段：`toast_count`、`toast_last_ts`（抽屉限流，字段名沿用代码）、`banner_since_ts`（重试卡起始，判 30s 升级）、`class_session_id`（进课清零计数）。
...
| 断线 → 阻断页 | 立即（1 次回调） | 重试卡立即 + **30s** 后升级 | 给 SDK 自动重连留时间 |
...
| C-P2 | Tier 2 全屏中断页 + 重试卡 + HTTP 请求分级静默化 | 替换存量 overlay，弹窗全量收敛 |
...
| 抽屉 / 重试卡组件是否已有现成实现 | UI | C-P1/P2 排期 |
