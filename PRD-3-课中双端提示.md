...
| **重试 UI**（横幅） | 半阻断 | 挂画面顶部、**系统自救中**、进度可见（第 N 次）、无按钮、无需操作 | 「网络中断 · 正在自动重连…」（画面定格）/「课程数据加载失败 · 正在重试…」（画面继续） | **只有两个**：成功 → 静默收起（+恢复 toast）；超时/耗尽 → 升级阻断 UI |
...
| --- | --- | --- | --- | --- |
| 抽屉 | **底部抽屉（bottom sheet）**：贴圆底升起、顶部圆角 34px、底边被圆缘裁切——「从屏幕边缘长出来」 | mini bars（灯色）+ 单行文案 | 文案 24px、≤10 全角（底部圆弦窄）；抽屉高 ≈ 94px | 上滑升起 → 停留约 3s → 下滑收回；冷却 300s、≤2 次/课（恢复不计入） |
| 重试 UI | **屏幕中央双行胶囊卡**——挡在内容前但不占满屏，正是「半阻断」的空间表达 | 第 1 行事件（「网络中断」/「课程数据加载失败」）+ 第 2 行蓝色进度（「⟳ 自动重连中（第 N 次）…」/「⟳ 重试中（1/2）…」，**有限次数外显**，耗尽即升级） | 两行均 24px、事件行 ≤8 全角；1.5px 白 16% 描边——与报告卡同一视觉语言：**描边 = 进行中的容器** | 持续显示至成功（静默收起 + 恢复 toast）或超时/耗尽升级；spinner 匀速旋转；断线变体画面定格调暗 50%，数据变体画面继续 |
| 阻断 UI | 全屏 | 归因图标（断网 = 扇形断开 / 弱网 = 红 1 格 bars）→ 标题 32「课程已暂停」→ 归因副行 24 灰 → 重试进度行 24 蓝 → 实底按钮「WiFi 设置」 | 沿用全局三档字号（32/24/15）与按钮规范 | 恢复 → 自动回课 + 恢复 toast |
...
配套两条硬规则：① **单向升级**——事件只能沿 Toast → 重试 UI → 阻断 UI 变重，绝不反向（用户看到重试 UI 就知道"系统在忙"，看到阻断就知道"该我了"）；② **恢复即回落**——任何一级在网络恢复时自动消退，不需要用户确认。常驻角标（bars）不属于提示组件，是第 0 层的状态显示，与三组件并行存在。
