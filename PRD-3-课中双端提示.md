...
![设备端课中三组件：抽屉三态 / 重试卡两态 / 阻断页两归因变体](./images/界面-设备端课中.png)
...
| **重试 UI**（重试卡） | 半阻断 | 屏幕中央双行卡、**系统自救中**、进度可见（第 N 次）、无按钮、无需操作 | 「网络中断 + ⟳ 自动重连中（第 N 次）…」（画面定格）/「课程数据加载失败 + ⟳ 重试中（1/2）…」（画面继续） | **只有两个**：成功 → 静默收起（+恢复抽屉）；超时/耗尽 → 升级阻断 UI |
...
**字号标准（探索结论）**：沿用全局三档（32/24/15），课中只动用两档——**32 仅用于全屏页标题**（阻断 UI「课程已暂停」），**24 用于一切组件内文案、进度行、按钮**；15 元信息层课中无内容。**一致性原则：字号跟随内容层级，不跟随打扰级别**——同一句提示在轻度抽屉和中度重试卡里字号相同（24），升级感全部由容器形态与位置表达（贴底单行 → 居中双行 → 全屏）。若字号随级别放大，同一句文案会出现三种大小，且「大字 = 重要」会与「全屏标题 32」的既有语义打架；中度级别的信息增量靠**加第二行**（进度），不靠加大字号。
...
新增状态字段：`drawer_cnt`、`drawer_last_sec`（抽屉限流）、`retry_since_sec`（重试卡起始，判 30s 升级）、进课统一走 `wn_reset_all()` 清零计数（字段名与参考代码一致）。
