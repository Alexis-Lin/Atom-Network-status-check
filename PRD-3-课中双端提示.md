...
| 项 | 内容 |
| --- | --- |
| 版本 | V1.0（评审定稿，进入开发——两组件模型：抽屉[单行瞬时 / 双行驻留] + 全屏中断页；基于现状代码 `weak_network_manager.c` 的函数级改造方案） |
...
| --- | --- | --- | --- | --- |
| 抽屉 · 单行 | **穿透式底部抽屉**：比屏幕大、从屏下拉上来的面板——顶部两角 r50 完整可见，底边与两侧下段延伸出圆屏外（被圆缘裁掉），贴底。**几何取舍（定稿）**：约束「圆角圆心距屏心 + 半径 ≤ 233px」下，半径加大到 r50 时**宽度不妥协**（文字显示区优先），以单行可见高 142→156 抬高顶边满足几何 | mini bars（灯色）+ 单行文案，内容行居可见区中部 | 宽 360px、可见高 ≈ 156px；文案 24px、≤10 全角 | 升起 0.45s（ease-out）→ 停留约 2s（1–2s 区间取上限，OTA 可调）→ 收回 0.45s（ease-in）；冷却 300s、≤2 次/课（恢复不计入） |
| 抽屉 · 双行驻留 | **同一底部容器的加高形态**（宽 360px、可见高 ≈ 190px、顶角 r50 穿透式）——中度提示不再居中霸屏 | 第 1 行事件（「网络中断」+ 红 bars /「课程数据加载失败」）+ 第 2 行蓝色进度（「⟳ 自动重连中（第 N 次）…」/「⟳ 重试中（1/2）…」，**有限次数外显**，耗尽即升级） | 两行均 24px、事件行 ≤8 全角；无描边（与抽屉一致），「进行中」由蓝色进度行 + spinner 表达 | 直接双行升起（或单行原位长高）后**驻留**至成功（静默收回 + 恢复抽屉）或超时/耗尽升级；spinner 匀速旋转；断线变体画面定格调暗 50%，数据变体画面继续 |
