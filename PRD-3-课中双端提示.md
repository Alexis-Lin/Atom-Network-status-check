...
| 重试 UI | 顶部中央胶囊（top ≈ 92px，取圆弦足宽处）——**顶部 = 持续状态**，与角标同位互斥 | 旋转 spinner（蓝 `--dev-info`）+ 单行文案（可含「第 N 次」） | 24px、≤11 全角；1.5px 白 16% 描边——与报告卡同一视觉语言：**描边 = 进行中的容器** | 持续显示至成功（静默收起 + 恢复 toast）或超时升级；spinner 匀速旋转；断线变体画面定格调暗 50%，数据变体画面继续 |
...
**互斥规则**：重试 UI 显示期间角标隐藏（同位）、不再发 toast（状态已在屏）；阻断 UI 显示期间其余组件全部不出——任意时刻屏上最多一个提示组件。
