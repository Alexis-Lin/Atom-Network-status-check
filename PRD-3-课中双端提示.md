...
| 现状代码 | [`backup/现状代码-weak_network_manager.c`](./backup/现状代码-weak_network_manager.c)（2026-07 快照，本文 §2 逐条解读） |
...
| `notify_timeout`（HTTP 超时→overlay） | **改造** | 按 §2.1 分级：可延后请求静默化；关键请求先静默自动重试（复用重放机制），耗尽才 Tier 2 |
...
| HTTP mtype 清单与关键性分级 | 客户端 | §2.1 分级表落地 |
