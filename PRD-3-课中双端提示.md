...
| --- | --- | --- | --- | --- |
| **Toast** | 非阻断 | 自动消失（约 3s）、零按钮、冷却 300s + 每课 ≤2 次 | 「网络一般 · 已降低画质」/「网络较差 · 你的/课程画面可能卡顿」/「网络已恢复」 | 自动消失，无状态 |
...
| --- | --- | --- | --- | --- |
| Toast | 底部中央胶囊（bottom ≈ 58px）——**底部 = 瞬时通知** | mini bars（灯色）+ 单行文案 | 文案 24px、≤14 全角；胶囊高 ≈ 52px | 淡入上滑 → 约 3s 自动淡出；冷却 300s、≤2 次/课（恢复 toast 不计入） |
...
**位置语义（打扰级别的空间表达）**：**贴底 = 轻度瞬时**（toast）→ **居中 = 中度持续**（重试卡，挡在内容前）→ **全屏 = 阻断**；顶部恒为状态层（角标）。侵入面积随级别递增，位置本身就是语义。
...
**互斥规则**：重试卡显示期间不再发 toast（状态已在屏），角标保留（状态层与事件层并行）；阻断 UI 显示期间其余组件全部不出——任意时刻屏上最多一个提示组件。
