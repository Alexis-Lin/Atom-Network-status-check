...
- **A4.2 检测中**：标题「正在测速…」+ 副行「Wi-Fi: {SSID} · 节点 N / M」（按官方回调驱动，不做假进度）+ 报告卡内波形实时生长（↓ 亮白 = 下行、↑ 中灰 = 上行，见 A4.7）+「取消」（调用 `stopSpeedTest`）；按钮防重入（官方限制同一时间仅一个测速任务）；
- **A4.3 结果页信息分组（四组，组间距 > 组内距）**：① 结论（bars + 一句话）→ ② **报告卡**（波形定格 + 四指标一行：延迟 / 丢包 / ↓ 下行 / ↑ 上行，下行上行标签带曲线同色箭头；不达期望的项用灯色着色；-1 显示「—」；**卡内页脚 = SSID + 测试时间**，如「FITURE-5G · 今天 14:32」）→ ③ 影响 + 建议（仅黄 / 红：影响 = 对上课意味着什么，如「动作识别可能延迟」；建议 = 怎么办）→ ④ 操作。**一张报告卡 = 一份完整测速报告，截图即可反馈客服**（截图给人看，上报给系统看）；卡内字号缩小（近距操作 UI）。稳定性派生标签：样本方差超阈值时卡下方显示「网络波动较大」；
...
- **A4.7 波形图规范（时间密度 · 实时刷新 · 配色）**：
  - **配色（单色系）**：↓ 下行 = 亮白 `#F2F5F6`（主）、↑ 上行 = 中灰 `#78848B`（辅），图例与指标标签带同色箭头——**亮度分层 + 箭头才是主编码**，不依赖色相分辨（色盲天然安全）。上下行没有正式行业标准色，事实惯例只有「箭头 ↓↑ + 下行在前」是通用的（Ookla 等），颜色各家自定；曲线**禁用红 / 黄 / 绿**（三色已被红绿灯语义占用，V1.0 前的稿子上行曲线恰好撞了绿灯色值，已改），也不引入青紫等第二色相搭配——报告卡保持单色，让灯色成为页面里唯一的「颜色语言」；
  - **时间密度**：一个样本点 = 一次 TRTC `onSpeedTestResult` 进度回调，官方节奏约每 2–3 秒一点、整次 10–20 秒，共约 5–8 点；备选文件测速路径按 1 秒一个吞吐量桶采样（约 15–30 点）。点即真实样本，**只画折线不做平滑插值**（不虚构数据），且**每个样本位置画一个小圆点**（r ≈ 2.5px）——点标出「哪一刻真的回传过数据」，线只负责连接；
  - **实时刷新**：检测中每收到一个回调就在曲线右端追加一点（可 300ms 缓动画入），**最新一点放大显示**（r ≈ 4px，标记「刚回传」），x 轴 = 已用时长（0 → 预计总时长，定宽不滚动），虚线游标标记当前进度；回调间隔内数值不做假动画。带宽值在延迟阶段尚无效（-1），曲线从首个有效带宽样本开始画；
