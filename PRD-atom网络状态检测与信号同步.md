...
- **A4.7 波形图规范（时间密度 · 实时刷新 · 配色）**：
  ...
  - **无底框**：报告卡不加底色容器和描边，波形 / 指标 / 页脚直接铺在纯黑背景上，靠间距分组——圆屏面积小，多一层浅色框反而显碎；
  ...
  - **实时刷新**：检测中每收到一个回调就在曲线右端追加一点（可 300ms 缓动画入），**最新一点放大显示**（r ≈ 4px，标记「刚回传」），x 轴 = 已用时长（0 → 预计总时长，定宽不滚动），虚线游标标记当前进度；**波形下沿标注时间轴**：左右两端 `0s` / 总时长，中间注明采样间隔「每点 ≈ 2–3 秒」，让读图者不用猜横轴是什么；回调间隔内数值不做假动画。带宽值在延迟阶段尚无效（-1），曲线从首个有效带宽样本开始画；
