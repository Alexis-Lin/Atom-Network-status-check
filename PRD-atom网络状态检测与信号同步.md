...
- **A2.1** 磁贴内 WiFi 图标右侧**三态常显**小型三格 bars（3 格绿 / 2 格黄 / 1 格红）。**决策记录**：原方案绿灯隐藏 bars、图标回中（依据「绿灯静默」），评审后改为常显——①「绿灯静默」约束的是**主动打扰**（通知 / toast / 弹窗），常驻区的静态显示不构成打扰；②常显让用户对「这里能看网速质量」建立稳定预期，绿灯时 bars 消失反而会被疑为「没在检测 / 功能坏了」；③与通知栏常驻条目（绿灯也显示「网速良好」）行为一致。**决策记录**：不用图标弧格填充度表达质量（弧格 = 信号强度语义，「信号满格但宽带拥塞」时必然误导）；不用图标染色（与开关语义打架）；不用角标圆点（与全系统 bars 不一致 + 17px 过小 + 录制红点歧义）；
