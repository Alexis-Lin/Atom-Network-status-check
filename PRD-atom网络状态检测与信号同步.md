...
- **A4.2 检测中**：标题「正在测速…」+ 副行「Wi-Fi: {SSID} · 节点 N / M」（按官方回调驱动，不做假进度）+ 报告卡内波形实时生长（↓ 青 = 下行、↑ 紫 = 上行，见 A4.7）+「取消」（调用 `stopSpeedTest`）；按钮防重入（官方限制同一时间仅一个测速任务）；
...
- **A4.7 波形图规范（时间密度 · 实时刷新 · 配色）**：
  - **配色**：↓ 下行 = 青色 `#2DD4BF`，↑ 上行 = 紫色 `#A78BFA`，图例与指标标签均带同色箭头（色 + 箭头双编码，色盲可辨）。上下行没有正式行业标准色，但事实惯例是 Ookla Speedtest 的「青绿下行 / 紫色上行」+ 箭头，本方案沿用该心智；**禁用红 / 黄 / 绿**——三色已被红绿灯语义占用，曲线用灯色会造成「上行 = 绿灯」的误读（V1.0 前的稿子恰好踩了这个坑，已改）；
  - **时间密度**：一个样本点 = 一次 TRTC `onSpeedTestResult` 进度回调，官方节奏约每 2–3 秒一点、整次 10–20 秒，共约 5–8 点；备选文件测速路径按 1 秒一个吞吐量桶采样（约 15–30 点）。点即真实样本，**只画折线不做平滑插值**（不虚构数据）；
  - **实时刷新**：检测中每收到一个回调就在曲线右端追加一点（可 300ms 缓动画入），x 轴 = 已用时长（0 → 预计总时长，定宽不滚动），虚线游标标记当前进度；回调间隔内数值不做假动画。带宽值在延迟阶段尚无效（-1），曲线从首个有效带宽样本开始画；
