...
| 项 | 内容 |
| --- | --- |
| 版本 | V1.1（界面定稿版：报告卡细框与标注、按钮槽位制、通知条目合并；含 2026-07-07 会议决议） |
| 日期 | 2026-07-07 |
| 状态 | 评审中 |
...
- **A4.7 波形图规范（时间密度 · 实时刷新 · 配色）**：
  ...
  - **屏缘环 = 进度专用**：**取消结果页 / 首页 / 失败态的状态色环**，屏缘环只保留一种用法——检测中的旋转进度环（宽约 10px）。**决策记录**：状态色环与「bars 灯色 + 标题 + 指标着色」三重冗余，整圈纯色视觉份量过重，且持续挤压顶排元素（bars 反复顶到环上）；去掉后状态表达零损失，「出现环 = 正在测速」反而成为一条清晰的新语义；
...
**结果页设计规范标注**（七个分区的字号 / 颜色 / 规则速查，完整交互见 Demo 附录）：

![设计规范标注：结论区/表头/波形/时间轴/四指标/操作区文案/按钮](./images/界面-设计规范标注.png)
