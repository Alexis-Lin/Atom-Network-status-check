...
- **A4.4 按钮逻辑（位置恒定 + 主按钮 = 最短解决路径）**：**位置语义固定——左槽恒为「重测」，右槽恒为「OK / 前进」类动作**；主次不靠位置表达，靠样式表达（主按钮实底、次按钮描边 ghost）。「完成」统一改为「OK」（更短，中英文同形）；红灯右槽为 **WiFi 扇形图标按钮**（图标即「去 WiFi 设置」语义，省文字且宽度可控）：

| 状态 | 左槽（恒 = 重测） | 右槽（恒 = OK / 前进） | 主按钮（实底） | 理由 |
| --- | --- | --- | --- | --- |
| 首页 | — | 测速（单按钮居中） | 测速 | 本页唯一动作 |
| 检测中 | — | 取消（单按钮居中） | — | 等待可中止 |
| 结果 · 绿 | 重测（描边） | OK（绿实底） | OK | 没问题最短路径是离开；存疑可就地复测 |
