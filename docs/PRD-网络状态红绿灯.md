...
- [`决策参考与迭代过程.html`](./决策参考与迭代过程.html)——状态行位置对比、图标语言论证、迭代时间线（过程归档）
...
统一字形为三格阶梯 bars：**3 格绿 / 2 格黄 / 1 格红，全暗 = 检测中**。理由：① 格数 + 颜色双通道编码，色盲 / 强光 / 小尺寸可读；② 阶梯形与 WiFi 扇形（= 本地信号强度，用于网络详情页）明确区分——**扇形管信号，阶梯管网速**，两个概念两个字形；③ 圆点被否决：与「录制中 🔴」红点语义冲突（摄像头设备上致命歧义）。论证见 [`决策参考与迭代过程.html`](./决策参考与迭代过程.html) Part 2。
...
- **A1.1 状态行锚定已连接网络**：已连接网络自动置顶到 WiFi 开关正下方（iOS 式动态排序），**网速状态行紧贴其下**——灯挂在它描述的网络上，位置即语义（与手机端「灯随用户画面」同一原则）；其余曾配网络归入分隔线下「其他曾配网络」分组。样式：三格 bars（灯色）+ 网速状态 + 弱化灰色「测速 ›」，无卡片底。文案三态：**网速良好 / 网速一般 / 网速较慢**；「测速 ›」三态恒显（本页唯一测速入口，恒显保证可发现性）。位置三方案对比见 [`决策参考与迭代过程.html`](./决策参考与迭代过程.html) Part 1；
