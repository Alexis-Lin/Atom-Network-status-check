...
**配套文件**（见仓库 `docs/` 目录与根 README）：

- [`02-界面示意图-Demo.html`](./02-界面示意图-Demo.html)——全部界面示意图，可直接演示，支持中 / EN 切换
- [`03-PRD-带示意图-完整版.html`](./03-PRD-带示意图-完整版.html)——本文档的插图版（按功能编号组织）
- [`04-决策参考-图标语言-圆点vs三格bars.html`](./04-决策参考-图标语言-圆点vs三格bars.html)——图标字形决策论证
- [`05-决策参考-WiFi状态行位置.html`](./05-决策参考-WiFi状态行位置.html)——状态行位置三方案对比
- [`06-参考代码-网络检测状态机.c`](./06-参考代码-网络检测状态机.c)——检测调度 / 状态机 / TRTC 接线的 C 参考实现
...
统一字形为三格阶梯 bars：**3 格绿 / 2 格黄 / 1 格红，全暗 = 检测中**。理由：① 格数 + 颜色双通道编码，色盲 / 强光 / 小尺寸可读；② 阶梯形与 WiFi 扇形（= 本地信号强度，用于网络详情页）明确区分——**扇形管信号，阶梯管网速**，两个概念两个字形；③ 圆点被否决：与「录制中 🔴」红点语义冲突（摄像头设备上致命歧义）。论证见 [`04-决策参考`](./04-决策参考-图标语言-圆点vs三格bars.html)。
...
> 各显示位置共享模块 B 的同一状态机与数据，任何入口灯色一致。示意图见 [`02-界面示意图-Demo.html`](./02-界面示意图-Demo.html)。

### A1 WiFi 列表页（原 F2.2–F2.7）

- **A1.1 状态行锚定已连接网络**：已连接网络自动置顶到 WiFi 开关正下方（iOS 式动态排序），**网速状态行紧贴其下**——灯挂在它描述的网络上，位置即语义（与手机端「灯随用户画面」同一原则）；其余曾配网络归入分隔线下「其他曾配网络」分组。样式：三格 bars（灯色）+ 网速状态 + 弱化灰色「测速 ›」，无卡片底。文案三态：**网速良好 / 网速一般 / 网速较慢**；「测速 ›」三态恒显（本页唯一测速入口，恒显保证可发现性）。位置三方案对比见 [`05-决策参考`](./05-决策参考-WiFi状态行位置.html)；
...
**状态机**：首页 →「测速」→ 检测中（可取消）→ 按官方 `quality` 进入绿 / 黄 / 红结果页；失败 → 失败态。结果页「重新检测」直接回到检测中。

- **A4.1 首页 = 纯入口（测速前）**：灯色 bars + 一句话结论 + 副行数据时效（「刚刚更新」≤60 秒 /「N 分钟前更新」≤5 分钟）+ 主按钮「测速」。**不展示任何数字指标**——常驻探测没有真实带宽，数字只在测速后出现，「测速前 = 入口、测速后 = 报告」天然可区分；缓存超 5 分钟不显示旧灯色，直接「检测中…」并刷新；
- **A4.2 检测中**：标题「正在测速…」+ 真实节点进度「节点 N / M」（按官方回调驱动，不做假进度）+ 报告卡内波形实时生长（蓝 = 下行、绿 = 上行，每 2–3 秒一个样本点）+「取消」（调用 `stopSpeedTest`）；按钮防重入（官方限制同一时间仅一个测速任务）；
- **A4.3 结果页信息分组（四组，组间距 > 组内距）**：① 结论（bars + 一句话）→ ② **报告卡**（波形定格 + 四指标一行：延迟 / 丢包 / 下行 / 上行，下行上行标签带曲线同色标记；不达期望的项用灯色着色；-1 显示「—」）→ ③ 影响 + 建议（仅黄 / 红：影响 = 对上课意味着什么，如「动作识别可能延迟」；建议 = 怎么办）→ ④ 操作。**一张报告卡 = 一份完整测速报告，截图即可反馈客服**（截图给人看，上报给系统看）；卡内字号缩小（近距操作 UI）。稳定性派生标签：样本方差超阈值时卡下方显示「网络波动较大」；
...
| 首页 | 测速 | — | 本页唯一动作 |
| 检测中 | — | 取消 | 等待可中止 |
| 结果 · 绿 | 完成（退出） | 重新检测 | 没问题最短路径是离开；存疑可就地复测 |
| 结果 · 黄 | 重新检测 | 完成 | 建议是软动作（暂停下载），做完就地验证 |
| 结果 · 红 | 去 WiFi 设置 | 重新检测 | 建议是硬动作（换网 / 挪设备）；什么都不改就重测结果不会变 |
| 失败态 | 重新检测 | 完成 | 任务未完成，重试是唯一有效动作 |

- **A4.5 失败态**：`success = false` 时**不给任何灯色**（没测出来 ≠ 网络差），「检测未完成 / 请检查网络连接后重试」；`errMsg` 仅上报不上屏；
...
| **主动测速**（A4 小程序） | TRTC `startSpeedTest`（备选：自研文件测速） | 用户触发，约 10–20 秒 | 少量真实测速流量；**课中禁止** |
...
- 进度：按节点每 2–3 秒回调一个样本，整次约 10–20 秒，环形进度按 `finishedCount / totalCount` 真实驱动；波形逐点绘制（约 5–8 点）；
...
| 重连超时 / WiFi 断开（>30 秒） | 全屏中断页（唯一保留的阻断样式）：归因 + 自动重试可见 | 仅「去 WiFi 设置」 |
...
| 小程序首页 | 副行（时效） | 刚刚更新 / 3 分钟前更新 · 测速查看详情 | Updated just now / 3 min ago · test for details |
| 小程序首页 | 主按钮 | 测速 | Speed test |
| 小程序检测中 | 进度 / 图例 | 节点 3 / 6；下行 / 上行 | Node 3 / 6; Down / Up |
| 小程序结果 | 绿 / 黄 / 红 | 网速良好 / 网速一般 / 网速较慢 | Good / Fair / Slow speed |
| 小程序结果 | 影响（黄 / 红） | 动作识别可能偶尔延迟 / 动作识别与计数可能延迟 | Pose tracking may lag at times / Pose tracking and rep counts may lag |
| 小程序结果 | 建议（路由器弱 / 宽带弱） | 将 Atom 靠近路由器或更换 WiFi / 网络繁忙 · 暂停其他设备下载后重测 | Move Atom closer to the router or switch Wi-Fi / Network busy · pause other downloads |
| 小程序结果 | 波动标签（方差超阈值） | 网络波动较大 | Unstable connection |
| 小程序失败态 | — | 检测未完成 / 请检查网络连接后重试 | Test incomplete / Check the connection and retry |
| 小程序按钮 | — | 测速 / 重新检测 / 完成 / 取消 / 去 WiFi 设置 | Speed test / Retest / Done / Cancel / Wi-Fi Settings |
...
| 全屏中断页 | WiFi 断开 | 网络已断开 / 课程已暂停 · WiFi 已断开 / 正在自动重连（第 N 次）… / 去 WiFi 设置 | Wi-Fi disconnected / Class paused / Reconnecting (attempt N)… / Wi-Fi Settings |
